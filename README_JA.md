...
これらは、検索の比較計算でFalse扱いになるため、DBの内容を設計する際には十分注意してください。  
バックエンドやクラウドが関与する場合、タイムゾーンはUTCで統一しておくことをオススメします。  
または、UNIX Timeの整数値を使う方法も高速で利便性が高いです。  
実際にどのように動くのか詳しく確認したい場合、[テストコード](https://github.com/MasahideMori-SimpleAppli/delta_trace_db/blob/main/test/timezone_search_test.dart)を確認してください。  
...
このパッケージはシングルスレッドでの動作を前提としています。  
メモリが共有されない並列処理などでは、メッセージパッシングなどの追加の処理が必要になることに注意してください。  
